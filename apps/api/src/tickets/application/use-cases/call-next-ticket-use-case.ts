...
export class CallNextTicketUseCase {
  ...
  public async execute(organizationId: string, windowId: string, attendedBy: string): Promise<Ticket> {
    ...
    const activeTicket = await this.ticketRepository.findByWindowId(windowId);
    if(activeTicket) throw ResponseError.badRequest('window already has an active ticket');

    const ticket = await this.ticketRepository.findNextTicketByServiceCategoryId(window.serviceCategoryId);
    if(!ticket) throw ResponseError.badRequest('there are no tickets to call');
    
    const calledTicket = ticket.call(windowId, attendedBy);
    
    return this.ticketRepository.update(calledTicket);
  }
}
