...
export class StartAttentionUseCase {
  ...
  public async execute(organizationId: string, windowId: string): Promise<Ticket> {
    ...
    if(!ticket) throw ResponseError.badRequest('there are no tickets to attend');

    const attendedTicket = ticket.attend();

    return this.ticketRepository.update(attendedTicket);
  }
}
